...
# obs_vars_M2 = ["y_axis_0","y_axis_1","y_axis_2","x_axis_0","x_axis_1","x_axis_2"]
num_observables = len(obs_vars_M1_ONE)
num_sets_obs_M1 = 2
# num_observables = 3
# num_observables = 6
# num_procs = 2
...
def convert_to_qcir(tau, expressions, t1_ext, t2_ext, formula_cond, dict_choices_eachlayer):
    ...
    for e in expressions:
        ...
        if (int(curr_time) > 0):
            ...
            for s in c_list:
                if (s not in layer_choices_list):
                    layer_choices_list.append(s)

            if ((curr_time) != int(counter)):
                # dict_choices_eachlayer[tau+str(counter)] = list(layer_choices)
                dict_choices_eachlayer[tau+str(counter)] = layer_choices_list
                layer_choices_list=[]
                # layer_choices=set()
                counter +=1
...
eventually_formulas ={}
all_formula_pairs = []
def build_realtional_conditions(tau):
...
eventually_halt={}
eventually_both_halt={}
def build_terminating_traj(tau):
    # global observable_vars_formulas
    ...
    for i in range(len_longest_trajectory+1):
        ...
        # for B in curr_time_step_tau2:
            ...
            eventually_halt[tau_name] = [var_dict[M1_term], var_dict[M2_term]]

            both_term = build_AND2(var_dict[M1_term], var_dict[M2_term])
            eventually_both_halt[tau_name] = both_term
...
# #     print(key, "  ", value)
build_realtional_conditions(tau_name)
build_terminating_traj(tau_name)
...
# arr1 = [1, 2, 3]
# arr2 = [4, 5, 6]
# index=build_exclusive_OR(arr1)
# print("index:"+index)
#

eventually_formulas_constraints = []
eventually_both_halt_constraints = []
eventually_t1_halt_constraints = []
eventually_t2_halt_constraints = []
tau_exclusive = []

index = build_AND2(str(var_dict[tau_name+get_phi(0, 0, 0)]), eventually_formulas[tau_name+get_phi(0, 0, 0)])
...
for key, value in tau_dict_choices_eachlayer.items():
    ...
    t1_one_of_them_terminated=[]
    t2_one_of_them_terminated=[]
    # print(key)
    # print(value)
    for v in value:
        ...
        one_of_them_terminated.append(build_AND2(str(var_dict[v]), eventually_both_halt[v]))
        t1_one_of_them_terminated.append(build_AND2(str(var_dict[v]), eventually_halt[v][0]))
        t2_one_of_them_terminated.append(build_AND2(str(var_dict[v]), eventually_halt[v][1]))

    # index=build_OR_multi(one_of_them_is_true)
    eventually_formulas_constraints.append(build_OR_multi(one_of_them_is_true))
    eventually_both_halt_constraints.append(build_OR_multi(one_of_them_terminated))
    eventually_t1_halt_constraints.append(build_OR_multi(t1_one_of_them_terminated))
    eventually_t2_halt_constraints.append(build_OR_multi(t2_one_of_them_terminated))
...
tau_eventually_terminated= build_OR_multi(eventually_both_halt_constraints)
tau_t1_eventually_terminated= build_OR_multi(eventually_t1_halt_constraints)
tau_t2_eventually_terminated= build_OR_multi(eventually_t2_halt_constraints)
elapsed = time.time()
print("time elapsed: ", round((elapsed - start), 3), 's')
...
for traj in traj_vars_tau:
    ...
    index = build_AND2(var_dict[traj[0]], build_OR2(var_dict[traj[0]], var_dict[traj[1]]))
    tau_moving.append(index)
tau_always_move = build_AND_multi(tau_moving)

### NEW
tau_MOVE = []
for j in range(len_longest_trajectory):
    print(traj_vars_tau[j])
    traj = traj_vars_tau[j]
    # print(eventually_both_halt_constraints[j])
    # print(eventually_t1_halt_constraints)
    # print(eventually_t2_halt_constraints)
    t1_move = build_AND2(var_dict[traj[0]], NOT+str(eventually_t1_halt_constraints[j]))
    t2_move = build_AND2(var_dict[traj[1]], NOT+str(eventually_t2_halt_constraints[j]))
    one_of_them_move = build_OR2(t1_move, t2_move)
    moving_rule = build_OR2(eventually_both_halt_constraints[j], one_of_them_move)
    tau_MOVE.append(moving_rule)

tau_moving_rule = build_AND_multi(tau_MOVE)
# tau_moving_rule = build_AND2(build_AND_multi(tau_MOVE), tau_always_move)
...
# print(INNER)
F_halt_A=INNER[0]
F_halt_B=INNER[1].replace(")","")
print("haltA:" + F_halt_A)
print("haltB:" + F_halt_B)
...
TERM=str(tau_eventually_terminated)
TERM_T1=str(tau_t1_eventually_terminated)
TERM_T2=str(tau_t2_eventually_terminated)
FORMULA = str(global_eventually_formula)

# MOVE_TAU=tau_moving_rule
MOVE_TAU=tau_always_move
# MOVE_both=tau_always_move
# MOVE_TAU=build_AND2(TERM, tau_always_move)

# POS=build_AND3(tau_all_formulas, tau_exclusive_constraints,tau_eventually_terminated)
POS = build_AND3(INITIAL_CONDITION, tau_all_formulas, tau_exclusive_constraints)
...
# Q_tau="exists"
# Q_pos="exists"
# ENC = build_AND3(MOVE_TAU, FORMULA, NOT+str(TERM))
# FINAL_FORMULA = build_AND2(M1, build_AND2(M2, build_AND2(POS, ENC)))
# FINAL_FORMULA = build_AND2(M1, build_AND2(M2, build_AND2(MOVE_TAU, build_AND4(POS, NOT+TERM_T1, TERM_T2, FORMULA))))
...
# Q_tau="exists"
# Q_pos="exists"
# ENC = build_AND2(MOVE_TAU, FORMULA)
# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_AND2(POS, ENC)))


### CASE: EA A, but pos in E, GOOD
Q_M1="exists"
Q_M2="forall"
Q_tau="forall"
Q_pos="forall"
...
FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_IMPLIES(MOVE_TAU, build_IMPLIES(POS, FORMULA))))

# ENC = build_AND2(MOVE_TAU, FORMULA)
# FINAL_FORMULA = build_AND4(M1, build_AND2(M2, NOT+TERM_T1), ENC, POS)
# ENC = build_IMPLIES(MOVE_TAU, build_IMPLIES(TERM))
# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_AND2(ENC, POS)))

# ENC = build_IMPLIES(MOVE_TAU, build_IMPLIES(TERM))
# FINAL_FORMULA = build_AND3(M1, TERM_T1, build_IMPLIES(M2, build_IMPLIES(PHI, build_AND3(MOVE_TAU, POS, NOT+TERM))))

# FINAL_FORMULA = build_AND3(M1, TERM_T1, build_IMPLIES(M2, build_IMPLIES(TERM_T2, build_IMPLIES(MOVE_TAU, build_AND2(POS, FORMULA)))))

# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_IMPLIES(MOVE_TAU, POS)))
# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_IMPLIES(PHI, build_IMPLIES(MOVE_TAU, build_AND2(POS, TERM_T1)))))

# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_IMPLIES(PHI, build_AND2(POS, TERM_T1))))
# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_IMPLIES(PHI, build_IMPLIES(MOVE_TAU, build_AND2(POS, TERM_T1)))))
# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_IMPLIES(MOVE_TAU, build_IMPLIES(TERM, build_AND2(POS, FORMULA)))))
# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_IMPLIES(MOVE_TAU, build_AND3(build_OR2(TERM_T1, TERM_T2), POS, FORMULA))))

# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_IMPLIES(PHI, build_IMPLIES(MOVE_TAU, build_AND3(build_OR2(TERM_T1, TERM_T2), POS, FORMULA)))))

### MMMM
# FINAL_FORMULA = build_AND3(M1, F_halt_A, build_IMPLIES(M2, build_IMPLIES(F_halt_B, build_IMPLIES(MOVE_TAU, build_AND2(POS, FORMULA)))))

# FINAL_FORMULA = build_AND2(M1, build_AND2(M2, build_IMPLIES(PHI, build_AND3(MOVE_TAU, POS))))

# ENC = build_IMPLIES(MOVE_TAU, build_AND2(POS, FORMULA))
# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, ENC))


# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_AND2(ENC, POS)))
# FINAL_FORMULA = build_AND2(M1, build_AND2(M2, build_AND2(POS, FORMULA)))
# ENC = build_IMPLIES(MOVE_TAU, build_AND2(POS, FORMULA))
# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, ENC))


### CASE: EA A, but pos in A
# Q_M1="exists"
# Q_M2="forall"
# Q_pos="forall"
# Q_tau="forall"
# ENC = build_IMPLIES(MOVE_TAU, FORMULA)
# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, build_IMPLIES(POS, ENC)))
# ENC = build_IMPLIES(MOVE_TAU, build_IMPLIES(POS, FORMULA))
# FINAL_FORMULA = build_AND2(M1, build_IMPLIES(M2, ENC))

#### ADD Variable Quantification
header_M1=Q_M1+"("
for a in vars_A:
...
# FORALL_TRY="forall("
# for b in vars_B:
#     FORALL_TRY = FORALL_TRY + b + ","
# for v in vars_traj:
#     FORALL_TRY += str(var_dict[v]) + ","
# for v in vars_pos:
#     FORALL_TRY += str(var_dict[v]) + ","
# FORALL_TRY=FORALL_TRY[:-1]
# FORALL_TRY += ")"
# write_QCIR.write(FORALL_TRY + "\n")
...
v_traj = Q_tau+"("
for v in vars_traj:
    v_traj += str(var_dict[v]) + ","
v_traj = v_traj[:-1] ## remove last ','
v_traj += ")"
write_QCIR.write(v_traj+ '\n')

v_pos = Q_pos+"("
for v in vars_pos:
    v_pos += str(var_dict[v]) + ","
v_pos = v_pos[:-1] ## remove last ','
v_pos += ")"
write_QCIR.write(v_pos+ '\n')
