...
let rec nnf (phi:expression) : expression =
  let nnf_aux (phi:expression) : expression =
    ...
    | Neg (MOr (ls))    -> MAnd(List.map (fun x -> nnf (Neg(x))) ls)
    | Neg (MAnd(ls))    -> MOr (List.map (fun x -> nnf (Neg(x))) ls)
...
let rec nnf_fast (phi:expression) : expression =
  ...
  let rec floop psi cont =
    ...
    | Neg(And(e1,e2)) -> cont2 (Neg e1) (Neg e2) f_or
...
let rec nnf_formula_fast (f:formula) : formula =
  match f with
  | Let(x,a,b) -> Let(x,nnf_fast a,nnf_formula_fast b)
  | General(e) -> General(nnf_fast e)
  | x -> x
...
(* AUX *)
(* not tail recursive *)
let rec remove_dup2 (ls:variable list) : variable list =
    match ls with
    | [x] -> [x]
    | []  -> []
    | hd::tl -> hd::(remove_dup2 (List.filter (fun x -> not (String.equal hd x)) tl))
...
let rec get_let_ids (f:formula): variable list =
  let rec aux ls phi =
    match phi with
      Let(x,a,b) -> aux (x::ls) b
    | _ -> ls
  in
  aux [] f

(* not tail recursive *)
let rec get_let_ids2 (f:formula): variable list =
  match f with
...
and get_vars_expr2 (e:expression) : variable list =
  remove_dup (get_vars_expr_aux2 e)
and get_vars_aux2 (f:formula) : variable list =
  match f with
  | Let(x,a,b) -> remove_dup (get_vars_expr2 a @ get_vars_aux2 b)
...
and get_vars_expr_aux2 (e:expression) : variable list=
  ...
  | Literal(Atom(v)) -> [v]
  | Literal(NegAtom(v)) -> [v]
  | Neg(x)   -> get_vars_expr_aux2 x
  | Or(x,y)  -> (get_vars_expr_aux2 x ) @ (get_vars_expr_aux2 y) 
  | MOr(ls)  -> List.flatten (List.map get_vars_expr_aux2 ls)
  | And(x,y) -> (get_vars_expr_aux2 x) @ (get_vars_expr_aux2 y)
  | MAnd(ls) -> List.flatten (List.map get_vars_expr_aux2 ls)
  | Implies(x,y) -> (get_vars_expr_aux2 x) @ (get_vars_expr_aux2 y)
  | Iff(x,y)     -> (get_vars_expr_aux2 x) @ (get_vars_expr_aux2 y)
  | _ -> []
...
and get_vars_aux (f:formula) : variable list =
  match f with
  | Let(x,a,b) -> get_vars_expr a @ get_vars_aux b
  | CNF(ls) -> List.flatten (List.map get_vars_disjunctive_formula ls)
  | DNF(ls) -> List.flatten (List.map get_vars_conjunctive_formula ls)
...
let rec size_expr_aux n e =
  ...
  match e with
  | Neg(x)   -> size_expr_aux n x
  | Or(x,y)  -> size2  n x y
  | MOr(ls)  -> sizels n ls
  | And(x,y) -> size2  n x y
  | MAnd(ls) -> sizels n ls
  | Implies(x,y) -> size2 n x y
  | Iff(x,y)     -> size2 n x y
  | _            -> n + 1

let size_expr e = size_expr_aux 0 e
                
let size f =
  let rec size_aux n f =
    match f with
      Let (_,x,y) -> size_aux (size_expr x) y
