...
module NuSMVSystem =
    ...
    type VariableType = 
        ...
        /// Return all values of that type
    ...
    /// Try to find the largest type that subsumes two types
    ...
    /// Check if t1 and t2 have the same base type
    ...
    /// Checks if a given value conforms to a given type
    ...
    type Expression = 
        ...
        /// Print this expression. Used for debugging
        member this.Print = 
            match this with 
                | Var s -> s 
                | Const a -> 
                        ...
                        | SetValue x -> 
                            ...
                            |> List.reduce (fun x y -> 
                                match joinTypes x y with 
                                    | Some x -> x 
                                    | None -> 
                                        printfn "Error during NuSMV-to-Explicit compilation."
                                        printfn "Failed to join types."
                                        exit 1
                                )
                            |> SetType
                | Eq _ -> 
                ...
                | SetExp e -> 
                    ...
                    |> List.reduce (fun x y -> 
                            match joinTypesAndExtend x y with 
                                ...
                                | None -> 
                                    printfn "Error during NuSMV-to-Explicit compilation."
                                    printfn $"Failed to join types %A{x} and %A{y}."
                                    exit 1
                            )
                    |> SetType 
                | Case (cases) -> 
            ...
            match this with 
                ...
                | And(e1, e2) -> 
                    ...
                    match v1, v2 with 
                        ...
                        | _ -> 
                            printfn "Error during NuSMV-to-Explicit compilation."
                            printfn $"& is not supported for values {{%A{v1}}} and {{%A{v2}}}"
                            exit 1
                | Or(e1, e2) -> 
                    let v1 = e1.Eval env 
                    let v2 = e2.Eval env 
                ...
                | Neg e ->
                    ...
                    match v with 
                        ...
                        | _ -> 
                            printfn "Error during NuSMV-to-Explicit compilation."
                            printfn $"! is not supported for values {{%A{v}}}"
                            exit 1
                | Add(e1, e2) -> 
                    let v1 = e1.Eval env 
                    let v2 = e2.Eval env 
    ...
    type SmvProgram = 
        ...
        member this.CheckConsistent() = 
            // First assert that all variables are defined
            this.Vars 
            |> Set.iter (fun x -> 
                if this.VarTypes.ContainsKey x |> not then 
                    printfn $"NuSMV is inconsistent: Variable Type for %s{x} is not defined."
                    exit 1

                if this.Init.ContainsKey x |> not then 
                    printfn $"NuSMV is inconsistent: Init Condition for %s{x} is not defined."
                    exit 1

                if this.Next.ContainsKey x |> not then 
                    printfn $"NuSMV is inconsistent: Next Condition for %s{x} is not defined."
                    exit 1
                )

            // Check that the Type matches for the initial values
            this.Vars 
            |> Set.iter (fun x -> 
                let v = this.Init.[x]
                ...
                if isValueOfType v t || isValueOfType v (SetType t) then 
                    ...
                    printfn $"NuSMV is inconsistent: Initial Value for %s{x} is %A{v}, but the type is %A{t}. Type mismatch."
                    exit 1
            )

            // Check that the next types are correct
            this.Vars 
            |> Set.iter (fun x -> 
                let t = this.VarTypes.[x]
                let e = this.Next.[x]
                ...
                let rec typeLookup y = 
                    if this.VarTypes.ContainsKey y then 
                        ...
                        printfn $"NuSMV is inconsistent: Variable %s{y} is used in the next condition for variable %s{x}, but not defined."
                        exit 1
                ...
                if haveSameBaseType infert t || haveSameBaseType infert (SetType t) then 
                    ...
                    printfn $"NuSMV is inconsistent: The next expression for variable %s{x} has type %A{infert} but should have the same basetype as %A{t} or %A{SetType t}. Type mismatch."
                    exit 1
            )

    /// Convert a Program to an explicit-state system
    let convertProgramToTS (p : SmvProgram) (expressionList : list<Expression>) = 

        // Eval an expression and unfold definitions on the fly
        let rec evalWithDefinitions state (e : Expression) = 
            e.Eval (fun x -> 
                if Set.contains x p.Vars then 
                    ...
                    printfn $"Did not find a definition for |%s{x}|"
                    exit 1
                )

        // A state is a Map<String, VariableValue>
        let queue = new Queue<_>()
        let step = new Dictionary<_,_>()
        
        assert(p.Vars = Set(p.VarTypes.Keys))

        // There can be multiple initial states if 
        let initStates = 
            p.Vars 
            |> Set.toList
            |> List.map (fun name -> 
                // Eval the next command
                let v = p.Init.[name]

                let possibleValues = 
                    if isValueOfType v p.VarTypes.[name] then 
                        // This is a unique next value, so a singleton of all options
                        [v]
                    else 
                        match v with 
                            | SetValue v -> 
                                if List.forall (fun x -> isValueOfType x p.VarTypes.[name]) v then 
                                    ...
                                    printfn $"The value of the next expression {{%A{v}}} does not match its intended type {{%A{p.VarTypes.[name]}}}"
                                    exit 1
                            | _ -> 
                                printfn $"The value of the next expression {{%A{v}}} does not match its intended type {{%A{p.VarTypes.[name]}}}"
                                exit 1
        ...
        while queue.Count <> 0 do 
            ...
            let allSucs = 
                ...
                |> List.map (fun name -> 
                    ...
                    let nextValues = 
                        if isValueOfType v p.VarTypes.[name] then 
                            // This is a unqiue next value, so a singleton of all options
                            [v]
                        else 
                            match v with 
                                | SetValue v -> 
            ...
            step.Add(state, (allSucs, apEval))

            for s in allSucs do 
                if step.ContainsKey s |> not && queue.Contains s |> not then 
                    queue.Enqueue s

        {
            TransitionSystem.APs = expressionList;
            TransitionSystem.InitialStates = initStates;
            TransitionSystem.Step = step
        }
    ...
    module Parser = 
        ...
        let expParser = 
            ...
            do 
                expParserRef := oppParser.ExpressionParser
        ...
        let programParser = 
            pipe4
                ...
                (fun _ var (init, next) define ->   
                    ...
                    |> List.iter (fun (n, c) -> 
                        if c > 1 then 
                            printfn $"Error while parsing NuSMV Model: The initial condition for variable %s{n} is defined multiple times."
                            exit 1
                        )
                    ...
                    |> List.iter (fun (n, c) -> 
                        if c > 1 then 
                            printfn $"Error while parsing NuSMV Model: The next condition for variable %s{n} is defined multiple times."
                            exit 1
                        )
                        ...
                        SmvProgram.Init = 
                            ...
                            |> List.map 
                                (fun (n, e) -> 
                                    n, e.Eval 
                                        (fun x -> 
                                            printfn $"Error while parsing NuSMV Model: Only closed expression in init conditions. %s{x} is unbound in %s{n}, %A{e}"
                                            exit 1)
                                )
                            |> Map.ofList
...
module NuSMVSpecification = 

    /// Special Kind of property for symbolic systems
    type RelationalAtom = 
        | UnaryPred of NuSMVSystem.Expression * int
        | RelationalEq of NuSMVSystem.Expression * int * NuSMVSystem.Expression * int

    /// The atoms of symbolic HyperLTL formula are relational predicates
    ...
    type NamedRelationalAtom = 
        | NamedUnaryPred of NuSMVSystem.Expression * String
        | NamedRelationalEq of NuSMVSystem.Expression * String * NuSMVSystem.Expression * String
    ...
    type NamedSymbolicHyperLTL = 
        ...
        member this.ToSymbolicHyperLTL() = 
            ...
            {
                SymbolicHyperLTL.QuantifierPrefix = this.QuantifierPrefix |> List.map fst 
                LTLMatrix = this.LTLMatrix.Map 
                    (fun x -> 
                        match x with 
                            | NamedUnaryPred (e, n) -> 
    ...
    module Parser = 
        ...
        let private namedSymbolicHyperltlParser = 
            ...
            let unaryAtomParser = 
                pipe2
                    (skipString "*" >>. NuSMVSystem.Parser.expParser .>> spaces .>> skipChar '*')
            ...
            let relationalAtomParser = 
                pipe4
                    ...
                    (spaces >>. skipString "*" >>. NuSMVSystem.Parser.expParser .>> spaces .>> skipChar '*')
                    (spaces >>. skipChar '_' >>. (many1Chars letter))
            ...
            pipe2 
                Core.LTL.Parser.namedPrefixParser
                (Core.LTL.Parser.ltlParser atomParser)
