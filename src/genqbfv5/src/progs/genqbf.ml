...
let parse_Q_file () : formula =
  if !Args.is_initial_file_C then
    let ch = Stdlib.open_in !Args.initial_file_C in
     Parser.parse ch (EParser.letclause ELexer.norm)
  else
    raise(ArgsError("must provide initial state formula file for C with -Q file"))

let parse_W_file () : formula =
  if !Args.is_transition_file_C then
    let ch = Stdlib.open_in !Args.transition_file_C in
    Parser.parse ch (EParser.letclause ELexer.norm)
  else
    raise(ArgsError("must provide transition relation formula file for C with -W file"))


let parse_Z_file () : formula =
  if !Args.is_initial_file_D then
    let ch = Stdlib.open_in !Args.initial_file_D in
     Parser.parse ch (EParser.letclause ELexer.norm)
  else
    raise(ArgsError("must provide initial state formula file for D with -Z file"))

let parse_X_file () : formula =
  if !Args.is_transition_file_D then
    let ch = Stdlib.open_in !Args.transition_file_D in
    Parser.parse ch (EParser.letclause ELexer.norm)
  else
    raise(ArgsError("must provide transition relation formula file for D with -X file"))

let parse_C_file () : formula =
  if !Args.is_initial_file_E then
    let ch = Stdlib.open_in !Args.initial_file_E in
     Parser.parse ch (EParser.letclause ELexer.norm)
  else
    raise(ArgsError("must provide initial state formula file for E with -C file"))

let parse_V_file () : formula =
  if !Args.is_transition_file_E then
    let ch = Stdlib.open_in !Args.transition_file_E in
    Parser.parse ch (EParser.letclause ELexer.norm)
  else
    raise(ArgsError("must provide transition relation formula file for E with -V file"))
...
let parse_separate_files () : QBF.problem_desc =
  ...
  | Some qalt -> 
    { init_A     = parse_I_file();
      ...
      init_B     = parse_J_file();
      tr_B       = parse_S_file();
      init_C     = parse_Q_file(); (* parse model C *)
      tr_C       = parse_W_file(); (* parse transition C *)
      init_D     = parse_Z_file(); (* parse model D *)
      tr_D       = parse_X_file(); (* parse transition D *)
      init_E     = parse_C_file(); (* parse model E *)
      tr_E       = parse_V_file(); (* parse transition E *)      
