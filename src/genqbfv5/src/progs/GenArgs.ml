...
(* -Q initial_file *)
let is_initial_file_C = ref false
let initial_file_C = ref ""
let set_initial_file_C s =  is_initial_file_C := true ;  initial_file_C := s

(* -W transition_file *)
let is_transition_file_C = ref false
let transition_file_C = ref ""
let set_transition_file_C s =  is_transition_file_C := true ;  transition_file_C := s



(* -Z initial_file *)
let is_initial_file_D = ref false
let initial_file_D = ref ""
let set_initial_file_D s =  is_initial_file_D := true ;  initial_file_D := s

(* -X transition_file *)
let is_transition_file_D = ref false
let transition_file_D = ref ""
let set_transition_file_D s =  is_transition_file_D := true ;  transition_file_D := s

(* -C initial_file *)
let is_initial_file_E = ref false
let initial_file_E = ref ""
let set_initial_file_E s =  is_initial_file_E := true ;  initial_file_E := s

(* -V transition_file *)
let is_transition_file_E = ref false
let transition_file_E = ref ""
let set_transition_file_E s =  is_transition_file_E := true ;  transition_file_E := s
...
(* -F  AE|AA|EE|EA|EAA *)
let unrolling_format_list = ["AA"; "AE"; "EE"; "EA"; "EAA"; "EEA"; "AEA"; "EEE" ; "AEE"; "EAAE" ; "EAAEE" ; "EEAA" ; "AAE" ; "AAAE" ; "AEEE" ; "AAEE" ; "AAAEEE"; "EEEE"]
...
let set_unrolling_format (fmt:string) : unit =
  ...
  | "EEA" -> unrolling_format := Some EEA
  | "AEA" -> unrolling_format := Some AEA
  | "EEE" -> unrolling_format := Some EEE
  | "AEE" -> unrolling_format := Some AEE
  | "EAAE" -> unrolling_format := Some EAAE
  | "EAAEE" -> unrolling_format := Some EAAEE
  | "AAE" -> unrolling_format := Some AAE
  | "AAAE" -> unrolling_format := Some AAAE
  | "AEEE" -> unrolling_format := Some AEEE
  | "AAEE" -> unrolling_format := Some AAEE
  | "EEEE" -> unrolling_format := Some EEEE
  | "AAAEEE" -> unrolling_format := Some AAAEEE
...
let opts =
  [
    ...
    ("-Q", Arg.String set_initial_file_C, "input file for initial state for C");
    ("-W", Arg.String set_transition_file_C, "input file for transition relation for C");
    ("-Z", Arg.String set_initial_file_D, "input file for initial state for D");
    ("-X", Arg.String set_transition_file_D, "input file for transition relation for D");
    ("-C", Arg.String set_initial_file_E, "input file for initial state for E");
    ("-V", Arg.String set_transition_file_E, "input file for transition relation for E");
    ("-P", Arg.String set_property_file, "property file");
    ("-F", Arg.Symbol (unrolling_format_list,set_unrolling_format), "format of the unrolling: AA | AE | EE | EA | AAE | AAAE | EAA | EEA | AEA | EEE | AEE | EAAE | EAAEE | AAEE | AAAEEE");
